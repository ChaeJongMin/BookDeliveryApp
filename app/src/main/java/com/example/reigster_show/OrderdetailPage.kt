...
class OrderdetailPage : AppCompatActivity(), OnMapReadyCallback{
    ...
    lateinit var gecoder:Geocoder
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_detail_orderpage)
//        checkPermission()
        startProcess()
        if(intent.hasExtra("customer_id")&&intent.hasExtra("delivery_id")){
            customer_id=intent.getStringExtra("customer_id").toString()
            delivery_id=intent.getStringExtra("delivery_id").toString()
            Log.d("배달원",delivery_id+1+" "+customer_id)
        }
        gecoder= Geocoder(this)
                ...
                .get().addOnSuccessListener { doc ->
                    for(index in doc) {
                        if (!index.data["orderCheck"].toString().toBoolean()){
                            db.collection("Orders").document(index.id).update(
                                mapOf(
                                    "orderCheck" to true,
                                    "deliveryid" to delivery_id
                                )
                            ).addOnSuccessListener {
                                val args = Bundle()
                                args.putParcelable("libraryPoint", locpoint)
                                args.putParcelable("customerPoint", customerpoint)
                                var deliveryMap_intent =
                                    Intent(this, Deliveryman_ShowMap::class.java)
                                deliveryMap_intent.putExtra("locbundle", args)
                                deliveryMap_intent.putExtra("customerid", customer_id)
                                deliveryMap_intent.putExtra("deliveryid", delivery_id)
                                Log.d("배달원", delivery_id)
                                Log.d("배달 버튼", "작동")
                                startActivity(deliveryMap_intent)
                            }
                        }
                        else{
                            Toast.makeText(getApplicationContext(), "이미 주문중입니다.", Toast.LENGTH_SHORT).show()
                        }
                    }
                }

        }
        btn_detailOrder_cancel.setOnClickListener {
            var deliveryRedo_intent= Intent(this, DeliveryOrderList::class.java)
            deliveryRedo_intent.putExtra("id",delivery_id)
            startActivity(deliveryRedo_intent)
...
//                            removeMessages(0)
    ...
    fun force_priorMove(){
        ...
        back_intent.putExtra("id",delivery_id)
        ...
        db2.collection("customers").whereEqualTo("id", customer_id)
            .get().addOnSuccessListener { customdoc ->
                ...
                for (customindex in customdoc) {
                    ...
                    var destinlist=gecoder.getFromLocation(customerpoint.latitude,customerpoint.longitude,5)
                    var daddress=destinlist.get(0).getAddressLine(0)
                    Log.d("지오코딩 메시지",daddress)
                    detailOrderAddress.setText(daddress)
                }
    ...
    fun setLastLocation(lastLocation:Location){
        ...
        val initcameraPosition = CameraPosition.builder().target(initLatLNG).zoom(16.0f).build()
