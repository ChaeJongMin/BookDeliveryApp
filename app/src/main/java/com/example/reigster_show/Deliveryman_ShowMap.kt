...
class Deliveryman_ShowMap : AppCompatActivity(), OnMapReadyCallback {
    ...
    var init_route=0
    var time:Int=0
    var lib=Location("Lib")
    var libdistance=0
    var arrivelib=false
    var prevmy=Location("prev")
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (locbundle != null) {
            ...
            lib.longitude=locpoint.longitude
            lib.latitude=locpoint.latitude
            ...
            while (true) {
                if(startthread) {
                    ...
                    val polylineOptions2 =
                        PolylineOptions().width(10f).color(Color.RED).clickable(true)
                    val polylineLibrary =
                        PolylineOptions().width(10f).color(Color.BLUE).clickable(true)
                    latLngArrayList.clear()
                    Log.d("메시지", "스레드 실행")

                        var urlString=""

                        if(arrivelib)urlString = setUri(customerpoint)
                        else if(!arrivelib) urlString = setUri(locpoint)
...
//                            textTotalDistance.setText(distance.toString()+"km")
                        ...
                        if(!arrivelib) polylineOptions2.addAll(latLngArrayList)
                        else polylineLibrary.addAll(latLngArrayList)

                    Thread.sleep(1000)
                    runOnUiThread {
                        //새로운 경로 그리기 전에 삭제
                        Log.d("메시지", "강제클릭 실행 전")
                        if(!arrivelib) polylineOptions = polylineOptions2
                            else polylineOptions = polylineLibrary
    ...
    fun setLastLocation(lastLocation: Location) {
        ...
        db.collection("DeliveryMan").whereEqualTo("id",delivery_id).get().addOnSuccessListener { doc->
            for(index in doc){
                ...
                //배달원-도서관 거리
                val my=Location("me")
                my.latitude=LatLNG.latitude
                my.longitude=LatLNG.longitude
                libdistance= my.distanceTo(lib).toInt()
                Log.d("두 지점 거리 메시지","거리: "+libdistance)
                if(libdistance <=30){
                    Toast.makeText(getApplicationContext(), "도서관에 곧 도착합니다.", Toast.LENGTH_SHORT).show()
                    arrivelib=true
                }
                if(libdistance <=10){
                    Toast.makeText(getApplicationContext(), "고객 위치에 대한 경로를 출력합니다.", Toast.LENGTH_SHORT).show()
                    arrivelib=true
                }
                if(count!=0){
                    bearing=my.bearingTo(prevmy)
                }
                ////////////////////
                val markerOptions = MarkerOptions().position(LatLNG).title("내 위치").flat(true)
                ...
                Log.d("메시지","시점 이동")
                count++
                prevmy.longitude=LatLNG.longitude
                prevmy.latitude=LatLNG.latitude
                startthread=true
            }
        }
    }
    fun setUri(points:LatLng): String {
        ...
        val urlString =
            "https://apis.openapi.sk.com/tmap/routes?version=1&format=json&callback=result&appKey=l7xx7c351c7720cb4c108b5c3f5cd5538575+&startX=" +
                    points.longitude+"&startY="+ points.latitude + "&endX=" + deliverypoint.longitude + "&endY=" + deliverypoint.latitude
