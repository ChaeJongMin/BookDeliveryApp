...
//    var OrderList = arrayListOf<Orders>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if(intent.hasExtra("id")){
            deliveryid=intent.getStringExtra("id").toString()
            Log.d("배달원 아이디", deliveryid)
        }

        threadstart = 0
        val adpater = OrdersAdapter(deliveryid)
        this.recyclerView.adapter = adpater
...
//                adpater.updateReceiptsList(templist)
    ...
    class OrdersAdapter(id:String) : RecyclerView.Adapter<OrdersAdapter.OrderHolder>() {
        ...
        override fun onBindViewHolder(holder: OrderHolder, position: Int) {
            ...
            if (!OrderList.get(position).orderCheck) {
                ...
                holder.itemView.setOnClickListener {
                    ...
                    Log.d("배달원 아이디(뷰 핸들러)",deliveryid+1)
                    orderPage_intent.putExtra("customer_id", OrderList.get(position).customerid)
                    orderPage_intent.putExtra("delivery_id", deliveryid)
